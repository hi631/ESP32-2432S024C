...
void setup() {
  // Use serial port
  Serial.begin(115200);
  pinMode(27, OUTPUT); digitalWrite(27, HIGH);  // BL On
...
void loop(void) {
  ...
  if (pressed==true) {
    ...
    //for (int i=0; i<ti.count; i++){
      ...
      Serial.println(' ');
    //}
    pressed = true; t_x = ti.x[i]; t_y = ti.y[i]-8; // y.offset(no.cal)
    //tft.drawEllipse(t_x, t_y, 5, 5, TFT_WHITE);   enf// touch Point.Check
  }
